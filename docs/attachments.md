...
combined message, 2 MB for an inbound message body, 5 GB per mailbox, and 25 GB for
...
`.env.example`. Provider response hydration defaults to a 40 MB hard cap through
`INBOUND_EMAIL_MAX_PROVIDER_MESSAGE_BYTES`. MIME traversal is additionally capped by
`INBOUND_EMAIL_MAX_MIME_DEPTH` and `INBOUND_EMAIL_MAX_MIME_PARTS`.
...
before storage. Gmail preflights provider size metadata, streams its full
response only to the hydration cap, and enforces declared and actual aggregate
body limits plus iterative MIME traversal limits. Graph message and attachment
bodies are streamed only up to configured limits. IMAP walks nested MIME leaves
and treats named text parts, inline binary
